...
**タスクに無関係なファイルが変更・破壊されていないか確認してください。**
...
以下に該当する場合は**即座に修正**してください：

- [ ] Issueの要件に無関係なファイルが変更されていないか
- [ ] ファイルが意図せずスタブ（数行）に置き換えられていないか
- [ ] 大量の削除がある場合、その削除は意図的なものか

> ⚠️ **既知の事故パターン**: ファイルAを実装する際に `write` ツールやリダイレクト（`>`）で
> 無関係なファイルBを誤って上書きし、数百行が3行のスタブに置き換えられる事故が発生しています。
