...
**重要**: `run:` ステップ（shellcheck, batsテストなど）は**PHASE_COMPLETEマーカー出力後に自動実行**されます。手動で `./scripts/test.sh` や `shellcheck` を実行すると、ワークフローがスキップされて品質チェックがバイパスされてしまいます。

- ✅ **やること**: テストコードの追加・修正、簡易的な動作確認（必要に応じて特定のテストのみ）
- ❌ **やらないこと**: 完全な品質チェック（shellcheck, 全テストスイート）の手動実行
