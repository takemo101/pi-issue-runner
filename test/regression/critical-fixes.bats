...
@test "Issue #22: mux_generate_session_name with pi-issue prefix" {
...
@test "Issue #22: mux_generate_session_name with dev prefix" {
    source "$PROJECT_ROOT/lib/config.sh"
    source "$PROJECT_ROOT/lib/multiplexer.sh"
...
@test "Issue #22: extract from dev-issue-99" {
    ...
    result="$(mux_extract_issue_number "dev-issue-99")"
    [ "$result" = "99" ]
}
...
@test "Issue #22: extract from custom-issue-123-bugfix" {
    ...
    result="$(mux_extract_issue_number "custom-issue-123-bugfix")"
    [ "$result" = "123" ]
}
...
@test "Issue #22: round-trip for issue 1" {
    ...
    session="$(mux_generate_session_name "1")"
    extracted="$(mux_extract_issue_number "$session")"
    [ "$extracted" = "1" ]
}
...
@test "Issue #22: round-trip for issue 42" {
    ...
    session="$(mux_generate_session_name "42")"
    extracted="$(mux_extract_issue_number "$session")"
    [ "$extracted" = "42" ]
}
...
@test "Issue #22: round-trip for issue 99" {
    ...
    session="$(mux_generate_session_name "99")"
    extracted="$(mux_extract_issue_number "$session")"
    [ "$extracted" = "99" ]
}
...
@test "Issue #22: round-trip for issue 123" {
    ...
    session="$(mux_generate_session_name "123")"
    extracted="$(mux_extract_issue_number "$session")"
    [ "$extracted" = "123" ]
}
