...
### 1. 品質ゲート（run:/call: ステップ）

ワークフローの任意の位置にシェルコマンド実行（`run:`）や別ワークフロー呼び出し（`call:`）を挿入できます。
AIステップの間に ShellCheck やテスト実行を挟むことで、マージ前の品質を担保します。

詳細は [workflows.md](workflows.md#run--call-ステップ非aiステップ) を参照。

### 2. 観点別コードレビュー

`call:` ステップを使って、専門的な観点からのレビューを別AIインスタンスで実行できます：

| エージェント | 観点 |
|-------------|------|
| `review-bugs.md` | バグ・ロジックエラー |
| `review-security.md` | セキュリティ |
| `review-architecture.md` | 設計・構造 |
| `review-ai-antipattern.md` | AI実装アンチパターン |

詳細は [README.md の観点別レビューセクション](../README.md#観点別レビューエージェント) を参照。
