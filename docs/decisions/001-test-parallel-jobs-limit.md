...
# 変更後（Issue #1144で最適化、CI実績あり）
local jobs="${BATS_JOBS:-4}"
```

### 理由
- 4並列ジョブはCI環境（Ubuntu）とローカル環境（macOS）で安定して動作することを確認
- CIでは既に `--jobs 4` で全テストを実行し、安定稼働している
- 2ジョブから4ジョブへの変更でテスト全体の実行時間を約30-40%短縮
- 環境変数 `BATS_JOBS` で上書き可能（柔軟性を保持）
- 16ジョブは依然としてハングの原因となるため非推奨
...
- `BATS_FAST_MODE=1`: 重いテストをスキップして高速実行
