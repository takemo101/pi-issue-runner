...
│  │  - 設定読み込み│  - worktree作成  │  - セッション管理 │  │
│  ├────────────────┼──────────────────┼───────────────────┤  │
│  │   github.sh    │    status.sh     │    workflow.sh    │  │
│  │  - Issue取得   │  - 状態管理      │  - ワークフロー   │  │
│  ├────────────────┼──────────────────┼───────────────────┤  │
│  │    log.sh      │    notify.sh     │    agent.sh       │  │
│  │  - ログ出力    │  - 通知機能      │  - マルチエージェン│  │
│  ├────────────────┼──────────────────┼───────────────────┤  │
│  │ dependency.sh  │    hooks.sh      │   template.sh     │  │
│  │  - 依存関係解析│  - Hook機能      │  - テンプレート   │  │
│  ├────────────────┼──────────────────┼───────────────────┤  │
│  │ workflow-finder│ workflow-loader  │  workflow-prompt  │  │
│  │  - 検索        │  - 読み込み      │  - プロンプト生成 │  │
│  ├────────────────┼──────────────────┼───────────────────┤  │
│  │  ci-classifier │    ci-fix.sh     │   ci-monitor.sh   │  │
│  │  - 失敗分類    │  - 自動修正      │  - 状態監視       │  │
│  ├────────────────┼──────────────────┼───────────────────┤  │
│  │   ci-retry.sh  │ cleanup-orphans  │  cleanup-plans    │  │
│  │  - リトライ    │  - 孤立削除      │  - 計画ローテーション│ │
│  ├────────────────┴──────────────────┴───────────────────┤  │
│  │                    yaml.sh                              │  │
│  │                   - YAMLパーサー                        │  │
