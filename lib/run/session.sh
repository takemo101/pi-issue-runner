...
# Simplified version - no step groups, no PHASE_COMPLETE.
# ============================================================================
...
run_session_start_hook() {
    ...
    # フック実行（Issue #1097: ラベルとワークフロー名を渡す）
    # トラッカーメタデータからワークフロー名を取得
    local workflow_name
    workflow_name="$(get_tracker_metadata "$issue_number" "workflow" 2>/dev/null || echo "default")"
    local session_label
    session_label="$(get_tracker_metadata "$issue_number" "label" 2>/dev/null || echo "")"
    
    run_hook "on_start" "$issue_number" "$session_name" "$branch_name" "$full_worktree_path" "$issue_title" "$session_label" "$workflow_name"
}

# ============================================================================
# Main entry point: Start agent session
...
# Export functions for use by run.sh
export -f start_agent_session
export -f generate_session_prompt
export -f apply_session_agent_config
export -f create_agent_session
export -f save_session_metadata
export -f run_session_start_hook
